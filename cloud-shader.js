/**
 * =======================================================================
 * IGNITO CORPORATION - FULLPAGE GLSL CLOUD SHADER BACKGROUND ☁️✨
 * =======================================================================
 */
...
`;
  ...
  function initCloudBackground() {
    let canvas = document.getElementById('cloud-shader-bg');
    if (!canvas) {
      canvas = document.createElement('canvas');
      canvas.id = 'cloud-shader-bg';
    ...
    function draw(now) {
      ...
      // Smooth color lerp
      for (let i = 0; i < 3; i++) {
        currentCloud[i] += (targetCloud[i] - currentCloud[i]) * 0.04;
        currentSkyTop[i] += (targetSkyTop[i] - currentSkyTop[i]) * 0.04;
        currentSkyBottom[i] += (targetSkyBottom[i] - currentSkyBottom[i]) * 0.04;
      }

      gl.uniform1f(loc.time, elapsed);
      gl.uniform1f(loc.count, 6.0);
      gl.uniform3f(loc.cloud, currentCloud[0], currentCloud[1], currentCloud[2]);
      gl.uniform3f(loc.skyTop, currentSkyTop[0], currentSkyTop[1], currentSkyTop[2]);
      gl.uniform3f(loc.skyBottom, currentSkyBottom[0], currentSkyBottom[1], currentSkyBottom[2]);
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initCloudBackground);
  } else {
    initCloudBackground();
  }
})();
